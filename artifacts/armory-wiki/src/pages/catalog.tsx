...
export default function Catalog() {
  ...
  const weaponsList = Array.isArray(weapons.data) ? weapons.data : (weapons.data as any)?.data ?? [];
  const suggestions = useMemo(() => weaponsList.slice(0, 6), [weaponsList]);
  ...
  return (
    <div className="mx-auto max-w-[1280px] px-5 pb-20 pt-12 lg:px-8 lg:pt-18">
      ...
      <div className="mt-10 flex items-center justify-between border-b border-border pb-4"><div className="flex items-center gap-3 font-meta text-[10px] uppercase tracking-[0.14em] text-muted-foreground"><SlidersHorizontal size={15} className="text-primary" /> {weapons.isLoading ? 'Сверяем полку…' : `${weaponsList.length} результатов`}{activeFilters > 0 && ` · ${activeFilters} фильтра`}</div><button type="button" onClick={reset} data-testid="button-reset-catalog" className="font-meta text-[10px] uppercase tracking-[.12em] text-muted-foreground hover:text-primary">Сбросить</button></div>
      {weapons.isLoading ? <div className="mt-5 grid gap-4 md:grid-cols-2"><WeaponCardSkeleton /><WeaponCardSkeleton /></div> : weapons.isError ? <div className="paper-panel mt-5 flex min-h-[280px] flex-col items-center justify-center rounded-sm px-6 text-center"><div className="stamp px-3 py-2">Ошибка соединения</div><h2 className="mt-5 font-display text-3xl">Полка временно закрыта</h2><p className="mt-2 text-sm text-muted-foreground">Не удалось получить записи. Попробуйте обновить запрос.</p><button type="button" onClick={() => weapons.refetch()} data-testid="button-retry-catalog" className="mt-6 bg-primary px-5 py-3 font-meta text-[10px] uppercase tracking-[.13em] text-primary-foreground">Повторить</button></div> : weaponsList.length > 0 ? <div className="mt-5 grid gap-4 md:grid-cols-2">{weaponsList.map((weapon, index) => <WeaponCard key={weapon.slug} weapon={weapon} index={index} />)}</div> : <div className="paper-panel mt-5 flex min-h-[280px] flex-col items-center justify-center rounded-sm px-6 text-center"><Search size={22} className="text-primary" /><h2 className="mt-5 font-display text-3xl">Такой записи пока нет</h2><p className="mt-2 max-w-sm text-sm leading-6 text-muted-foreground">Проверьте точное название или очистите один из фильтров.</p><button type="button" onClick={reset} data-testid="button-empty-reset" className="mt-6 font-meta text-[10px] uppercase tracking-[.13em] text-primary hover:underline">Сбросить фильтры</button></div>}
